...
export const contentApi = {
  ...
  getList(params?: {
    ...
    return request.get('/entry', { params })
  },
  
  // 获取内容条目详情
  getDetail(id: number): Promise<Entry> {
    return request.get(`/entry/${id}`)
  },
  ...
  delete(id: number): Promise<void> {
    return request.delete(`/entry/${id}`)
  },
  
  // 发布内容条目
  publish(id: number): Promise<Entry> {
    return request.put(`/entry/${id}/publish`)
  },
  
  // 取消发布内容条目
  unpublish(id: number): Promise<Entry> {
    return request.put(`/entry/${id}/unpublish`)
  },
}